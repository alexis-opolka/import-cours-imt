...
void affiche_vehicules(int nombre, Vehicule* vehicules){
  for (int i = 0; i < nombre; i++){
     if (vehicules[i].type == BATEAU){
       ...
       affiche_voiture(&vehicules[i].infos.voiture_infos);
     }
  }
}
...
Vehicule cree_voiture(int puissance, int essence_restante, float usage_pneus){
  Vehicule v;
  v.type = VOITURE;
  v.infos.voiture_infos.essence_restante = essence_restante;
  v.infos.voiture_infos.puissance = puissance;
  v.infos.voiture_infos.usage_pneus = usage_pneus;
  return v;
}
